...
recompute, and Claude Code's \texttt{/compact} boundary. Readers
familiar with the first three may skim to \S\ref{sec:bg-claudecode};
the \texttt{/compact} pattern is what makes the rest of the paper a
...
a single recompute ratio. \emph{We pick up the methodological thread
here}: agent traffic is not RAG, $T=0$ greedy decoding makes
single-token differences load-bearing, and hit rate is not output
quality. \S\ref{sec:evaluation} measures all three.
