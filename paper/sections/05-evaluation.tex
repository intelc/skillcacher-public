...
questions admit numerical answers; the third is the question that
...
\item[\textbf{cacheblend}] lmcache enabled with our patches, cacheblend
  blending active. The interesting condition.
\end{description}
...
\paragraph{Two regimes.} Reading Table~\ref{tab:hit-rate} alongside
Figure~\ref{fig:rescue-distribution}, the rescue mechanism behaves
sharply differently in two regimes. In the \emph{steady-state regime}
...
\textbf{The deployment-relevant story is the reverse of the published
narrative}: the turns where cacheblend rescues the most prefill tokens
(95--99\% hit rate) are also the turns where it pays the largest
end-to-end latency cost. On the 6 turns reaching $\geq$99\% rescue,
...
the median and 7$\times$ slower at the peak. Hit rate is not output
quality, and on this setup it is also not latency.
...
Read median + IQR together: \emph{median identity is 1.0000 because more
than half of substantive turns are bit-identical, but the IQR's lower
quartile sitting at 0.11 means a quarter of turns drop to roughly 10\%
identity.} This is not a ``$\geq$99\% gate met'' finding; the divergence
...
The dual-mode picture rules out the simplest reading of cacheblend's own
quality claim (``small but bounded drift''). On Llama-70B fp8 with the
default \texttt{recompute\_ratios}=0.15, the drift is sharply bimodal:
mostly zero, occasionally large.
...
could in principle be ``noise that mostly washes out'' --- if cacheblend's
divergent output were on average just as task-useful as no\_cache's, the
identity-rate degradation would be a meaningless artefact. We test this
directly with an LLM-judge preference rate.
...
\paragraph{Result.} Cacheblend fails the spec's $\geq$85\% non-inferiority
gate cleanly:
...
aggregate impression across an entire session. We argue the
conditional rate is the more load-bearing measurement, since the
bit-identical pairs do not exercise the cacheblend code path that
matters; reviewers may reasonably prefer either framing.
...
\paragraph{The judge is doing substantive work, not pattern-matching.}
The per-pair rationales show real evaluation, not a mechanical
``prefer tool calls'' heuristic. On
